...
async def test_add_travel_duplicate_id() -> None:
    ...
    with patch.object(TravelService, '__init__', return_value=None):
        ...
        with pytest.raises(ValueError, match=re.escape("Путешествие c таким ID уже существует.")):
...
async def test_search_travels_not_found() -> None:
    ...
    with patch.object(TravelService, '__init__', return_value=None):
        ...
        with pytest.raises(ValueError, match=re.escape("Путешествие по переданным параметрам не найдено.")):
...
async def test_complete_travel_error() -> None:
    ...
    with patch.object(TravelService, '__init__', return_value=None):
        ...
        with pytest.raises(ValueError, match=re.escape("Ошибка при завершении путешествия")):
            await service.complete(1)
...
async def test_get_users_by_travel_error() -> None:
    ...
    with patch.object(TravelService, '__init__', return_value=None):
        ...
        with pytest.raises(ValueError, match=re.escape("Ошибка при получении пользователей")):
...
async def test_get_entertainments_by_travel_error() -> None:
    ...
    with patch.object(TravelService, '__init__', return_value=None):
        ...
        with pytest.raises(ValueError, match=re.escape("Ошибка при получении развлечений для путешествий")):
...
async def test_get_accommodations_by_travel_error() -> None:
    ...
    with patch.object(TravelService, '__init__', return_value=None):
        ...
        with pytest.raises(ValueError, match=re.escape("Ошибка при получении завершенных путешествий")):
...
async def test_link_entertainments_success() -> None:
    ...
    with patch.object(TravelService, '__init__', return_value=None):
        ...
        service.repository.link_entertainments.assert_called_once_with(1, entertainment_ids)
...
async def test_link_entertainments_error() -> None:
    ...
    with patch.object(TravelService, '__init__', return_value=None):
        ...
        with pytest.raises(ValueError, match=re.escape("Ошибка при связывании развлечений с путешествием.")):
            await service.link_entertainments(1, entertainment_ids)
...
async def test_link_users_error() -> None:
    ...
    with patch.object(TravelService, '__init__', return_value=None):
        ...
        with pytest.raises(ValueError, match=re.escape("Ошибка при связывании пользователей с путешествием.")):
            await service.link_users(1, user_ids)
...
async def test_link_accommodations_success() -> None:
    ...
    with patch.object(TravelService, '__init__', return_value=None):
        ...
        service.repository.link_accommodations.assert_called_once_with(1, accommodation_ids)
...
async def test_link_accommodations_error() -> None:
    ...
    with patch.object(TravelService, '__init__', return_value=None):
        ...
        with pytest.raises(ValueError, match=re.escape("Ошибка при связывании мест проживания с путешествием.")):
            await service.link_accommodations(1, accommodation_ids)
...
async def test_get_travels_for_user_error() -> None:
    ...
    with patch.object(TravelService, '__init__', return_value=None):
        ...
        with pytest.raises(ValueError, match=re.escape("Ошибка при получении активных путешествий")):
