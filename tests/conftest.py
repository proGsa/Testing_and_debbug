...
async def fill_test_data(session: AsyncSession) -> None:
    ...
    for data in d_routes:
        await session.execute(
            text(
                "INSERT INTO directory_route (type_transport, departure_city, \
                arrival_city, distance, price) \
            VALUES (:type_transport, :departure_city, :arrival_city, :distance, :price)"
            ),
            data,
        )
    users = [
    ...
    travels_data = [
        {"status": "В процессе", "user_id": 1},
        {"status": "Завершен", "user_id": 1},
    ]
    tr_ent = [(1, 2), (2, 1)]
    tr_a = [(1, 1), (2, 2)]
    ...
    await session.execute(
        text(
            "INSERT INTO users_travel (travel_id, users_id) VALUES (:travel_id, :users_id)"
        ),
...
@pytest_asyncio.fixture
async def accommodation_repo(
    db_session: AsyncSession, city_repo: CityRepository
) -> AccommodationRepository:
    return AccommodationRepository(db_session, city_repo)


@pytest_asyncio.fixture
async def accommodation_service(
...
@pytest_asyncio.fixture
async def entertainment_service(
    entertainment_repo: EntertainmentRepository,
) -> EntertainmentService:
    return EntertainmentService(entertainment_repo)


@pytest_asyncio.fixture
async def directory_route_repo(
    db_session: AsyncSession, city_repo: CityRepository
) -> DirectoryRouteRepository:
    return DirectoryRouteRepository(db_session, city_repo)


@pytest_asyncio.fixture
async def d_route_service(
    directory_route_repo: DirectoryRouteRepository,
) -> DirectoryRouteService:
