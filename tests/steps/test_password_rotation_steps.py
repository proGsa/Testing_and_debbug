...
def given_password_expired(password_user):
    resp = requests.post(
        f"{BASE_URL}/api/expire-password",
        json={"login": password_user["login"]}
    )
...
def when_login_old_password(password_user):
    resp = requests.post(f"{BASE_URL}/api/login1",
                         json={"login": password_user["login"], "password": password_user["old_password"]})
...
def then_login_new_password(password_user):
    resp = requests.post(f"{BASE_URL}/api/login1",
                         json={"login": password_user["login"], "password": password_user["new_password"]})
    assert resp.status_code == 200
