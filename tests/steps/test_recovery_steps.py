...
def recover_user():
    ...
    requests.delete(f"{BASE_URL}/api/delete/{data['user_id']}",
...
def when_wrong_password(recover_user):
    ...
    for _ in range(5):
        resp = requests.post(f"{BASE_URL}/api/login1", json={"login": login, "password": "wrongpass"})
...
def when_blocked(recover_user):
    ...
    resp_blocked = requests.post(f"{BASE_URL}/api/login1", json={"login": login, "password": password})
...
def when_recover_password(recover_user):
    ...
    recover_resp = requests.post(f"{BASE_URL}/api/recover-password", json={"login": login})
...
def then_can_login(recover_user):
    ...
    resp_login = requests.post(f"{BASE_URL}/api/login1", json={"login": login, "password": password})
