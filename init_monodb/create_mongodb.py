...
class PostgreSQLToMongoMigrator:
    ...
    async def _migrate_cities(self, session: AsyncSession, db: MongoDB) -> None:
        ...
        result = await session.execute(text("SELECT city_id, name FROM city"))  # Note: added id here
    ...
    async def _migrate_directory_routes(self, session: AsyncSession, db: MongoDB) -> None:
    ...
    async def _migrate_travels(self, session: AsyncSession, db: MongoDB) -> None:
        ...
        result = await session.execute(text("SELECT travel_id, users_id FROM users_travel"))
        ...
        result = await session.execute(text("SELECT travel_id, accommodation_id FROM travel_accommodations"))
        ...
        result = await session.execute(text("SELECT travel_id, entertainment_id FROM travel_entertainment"))
    ...
    def _create_indexes(self, db: MongoDB) -> None:
        ...
        db.directory_routes.create_index([("departure_city_id", 1), ("arrival_city_id", 1)])


async def async_init_mongodb() -> None:
    """Асинхронная инициализация MongoDB"""
    pg_async_url = "postgresql+asyncpg://nastya:nastya@postgres_container_ppo:5432/mydb"
