...
class Travel(BaseModel):
    ...
    entertainments: list[Entertainment] = Field(
        default_factory=list, description="Список развлечения"
    )
    accommodations: list[Accommodation] = Field(
        default_factory=list, description="Список размещения"
    )
    ...
    def validate_status(cls, v: str) -> str:
        ...
        if v not in allowed_types:
            raise ValueError(
                f'e_type должен быть одним из следующих: {", ".join(allowed_types)}'
            )
        return v
    ...
    def check_entertainments(cls, value: list[Entertainment]) -> list[Entertainment]:
        if value is not None:
            ...
            if not all(isinstance(item, Entertainment) for item in value):
                raise ValueError(
                    "Bce элементы entertainments должны быть экземплярами Entertainment"
                )
    ...
    def check_accommodations(cls, value: list[Accommodation]) -> list[Accommodation]:
        if value is not None:
            ...
            if not all(isinstance(item, Accommodation) for item in value):
                raise ValueError(
                    "Bce элементы accommodation должны быть экземплярами Accommodation"
                )
