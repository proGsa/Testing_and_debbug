...
async def get_all_entertainments(
    ...
    return templates.TemplateResponse(
        "entertainment.html",
        {"request": request, "entertainments": entertainments, "cities": cities},
    )


@entertainment_router.get("/entertainment/get/{entertainment_id}")
async def get_entertainment(
    entertainment_id: int, service_locator: ServiceLocator = get_sl_dep
) -> dict[str, Any]:
    try:
        result = await service_locator.get_ent_contr().get_entertainment_details(
            entertainment_id
        )
        if result is None:
        ...
        logger.error(
            "Ошибка при получении информации о развлечении: %s", str(e), exc_info=True
        )
...
async def update_entertainment(
    ...
    result = await service_locator.get_ent_contr().update_entertainment(
        entertainment_id, request
    )
...
@entertainment_router.post(
    "/entertainment/delete/{entertainment_id}", response_class=HTMLResponse
)
async def delete_entertainment(
    ...
    result = await service_locator.get_ent_contr().delete_entertainment(
        entertainment_id
    )
...
@entertainment_router.delete(
    "/route/entertainment/delete/{entertainment_id}", response_class=HTMLResponse
)
async def delete_entertainment_for_route(
    entertainment_id: int, route_id: int, service_locator: ServiceLocator = get_sl_dep
) -> RedirectResponse:
    result = await service_locator.get_ent_contr().delete_entertainment(
        entertainment_id
    )
...
async def add_ent_to_route(
    ...
    try:
        ...
        ent_ids = []
        entertainments = (
            await service_locator.get_travel_serv().get_entertainments_by_travel(
                travel.travel_id
            )
        )
        ...
        await service_locator.get_travel_serv().link_entertainments(
...
async def update_entertainment_dates(
    ...
    return await service_locator.get_ent_contr().update_entertainment_dates(
        entertainment_id, request
    )
