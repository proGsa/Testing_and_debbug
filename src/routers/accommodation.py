...
async def get_all_accommodations(
    ...
    return templates.TemplateResponse(
        "accommodation.html",
        {"request": request, "accommodations": accommodations, "cities": cities},
    )


@accommodation_router.get("/accommodation/get/{accommodation_id}")
async def get_accommodation(
    accommodation_id: int, service_locator: ServiceLocator = get_sl_dep
) -> dict[str, Any]:
    try:
        acc = await service_locator.get_acc_contr().get_accommodation_details(
            accommodation_id
        )
        if acc is None:
        ...
        logger.error(
            "Ошибка при получении информации о проживании: %s", str(e), exc_info=True
        )
        raise HTTPException(status_code=500, detail=str(e))


@accommodation_router.put(
    "/api/accommodations/{accommodation_id}", response_class=HTMLResponse
)
async def update_accommodation(
    ...
    result = await service_locator.get_acc_contr().update_accommodation(
        accommodation_id, request
    )
...
@accommodation_router.post(
    "/accommodation/delete/{accommodation_id}", response_class=HTMLResponse
)
async def delete_accommodation(
    accommodation_id: int, service_locator: ServiceLocator = get_sl_dep
) -> RedirectResponse:
    result = await service_locator.get_acc_contr().delete_accommodation(
        accommodation_id
    )
...
@accommodation_router.delete(
    "/route/accommodation/delete/{accommodation_id}", response_class=HTMLResponse
)
async def delete_accommodation_for_route(
    accommodation_id: int, route_id: int, service_locator: ServiceLocator = get_sl_dep
) -> RedirectResponse:
    result = await service_locator.get_acc_contr().delete_accommodation(
        accommodation_id
    )
...
async def add_acc_to_route(
    route_id: int, request: Request, service_locator: ServiceLocator = get_sl_dep
) -> RedirectResponse:
    try:
        result = await service_locator.get_acc_contr().create_new_accommodation(request)
        logger.info("Размещение успешно создано: %s", result)
        travel = await service_locator.get_travel_serv().get_travel_by_route_id(
            route_id
        )
        if not travel:
        ...
        ent_ids = []
        accommodations = (
            await service_locator.get_travel_serv().get_accommodations_by_travel(
                travel.travel_id
            )
        )
        ...
        await service_locator.get_travel_serv().link_accommodations(
            travel.travel_id, ent_ids
        )
        return RedirectResponse(url=f"/route/edit/{route_id}", status_code=303)
...
async def update_accommodation_dates(
    ...
    return await service_locator.get_acc_contr().update_accommodation_dates(
        accommodation_id, request
    )
