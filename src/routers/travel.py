...
async def get_all_travels(
    ...
    user_id = (
        travels[0]["users"][0]["user_id"] if travels and travels[0]["users"] else None
    )
    user = None
    ...
    for travel in travels:
        for entertainment in travel["entertainments"]:
            if isinstance(entertainment.get("city"), dict):
                entertainment["city_name"] = entertainment["city"].get(
                    "name", "Undefined"
                )
        ...
        for accommodation in travel["accommodations"]:
            if isinstance(accommodation.get("city"), dict):
                accommodation["city_name"] = accommodation["city"].get(
                    "name", "Undefined"
                )
...
async def update_travel(
    travel_id: int, request: Request, service_locator: ServiceLocator = get_sl_dep
) -> HTMLResponse:
...
async def search_travel(
    ...
    logger.info(
        "Поиск завершен, найдено %d путешествий", len(result.get("travels", []))
    )
    return result
