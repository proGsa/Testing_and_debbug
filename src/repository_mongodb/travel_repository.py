...
class TravelRepository(ITravelRepository):
    ...
    async def get_accommodations_by_travel(self, travel_id: int) -> list[Accommodation]:
        try:
            ...
            logger.debug(
                "Получено %d размещений для путешествия ID %d",
                len(accommodations),
                travel_id,
            )
    ...
    async def get_users_by_travel(self, travel_id: int) -> list[User]:
        try:
            ...
            logger.error(
                "Ошибка при получении пользователей: %s", str(e), exc_info=True
            )
            return []
            ...
            async for doc in self.travels.find().sort("_id"):
                travels.append(
                    Travel(
                        ...
                        entertainments=await self.get_entertainments_by_travel(
                            int(doc["_id"])
                        ),
                        accommodations=await self.get_accommodations_by_travel(
                            int(doc["_id"])
                        ),
                    )
                )
    ...
    async def add(self, travel: Travel) -> Travel:
        try:
            ...
            doc = {
                ...
                "entertainments": [
                    ent.entertainment_id for ent in travel.entertainments
                ],
                "accommodations": [
                    acc.accommodation_id for acc in travel.accommodations
                ],
            }
    ...
    async def update(self, update_travel: Travel) -> None:
        try:
            result = await self.travels.update_one(
                {"_id": update_travel.travel_id},
                {
                    "$set": {
                        ...
                        "users": (
                            [user.user_id for user in update_travel.users]
            ...
            if result.modified_count == 0:
                logger.warning(
                    "Путешествие ID %d не найдено для обновления",
                    update_travel.travel_id,
                )
            else:
                logger.debug(
                    "Путешествие ID %d успешно обновлено", update_travel.travel_id
                )
        except PyMongoError as e:
    ...
    async def search(self, travel_dict: dict[str, Any]) -> list[Travel]:
        try:
            ...
            if "departure_city" in travel_dict:
                query["routes.d_route.departure_city_id"] = travel_dict[
                    "departure_city"
                ]
            ...
            if "entertainment_name" in travel_dict:
                query["entertainments.event_name"] = {
                    "$regex": travel_dict["entertainment_name"],
                    "$options": "i",
                }
    ...
    async def complete(self, travel_id: int) -> None:
        try:
            result = await self.travels.update_one(
                {"_id": travel_id}, {"$set": {"status": "Завершен"}}
            )
    ...
    async def link_entertainments(
        ...
        try:
            result = await self.travels.update_one(
                {"_id": travel_id}, {"$set": {"entertainments": entertainment_ids}}
            )
    ...
    async def link_accommodations(
        ...
        try:
            result = await self.travels.update_one(
                {"_id": travel_id}, {"$set": {"accommodations": accommodation_ids}}
            )
    ...
    async def get_travels_for_user(self, user_id: int, status: str) -> list[Travel]:
        try:
            travels = []
            async for doc in self.travels.find({"status": status, "users": user_id}):
                travels.append(
                    Travel(
                        travel_id=int(doc["_id"]),
                        status=doc["status"],
    ...
    async def link_users(self, travel_id: int, user_ids: list[int]) -> None:
        try:
            result = await self.travels.update_one(
                {"_id": travel_id}, {"$set": {"users": user_ids}}
            )
