...
class AccommodationRepository(IAccommodationRepository):
    ...
    async def get_list(self) -> list[Accommodation]:
        try:
            ...
            async for doc in self.collection.find().sort("_id"):
                ...
                if not city:
                    logger.warning(
                        f"Город с ID {doc['city_id']} не найден для размещения {doc['_id']}"
                    )
                    continue
    ...
    async def add(self, accommodation: Accommodation) -> Accommodation:
        try:
            ...
            logger.debug(
                "Размещение '%s' успешно добавлено с ID %d", accommodation.name, new_id
            )
    ...
    async def delete(self, accommodation_id: int) -> None:
        try:
            result = await self.collection.delete_one(
                {"_id": int(str(accommodation_id))}
            )
            if result.deleted_count == 0:
                logger.warning(
                    "Размещение с ID %d не найдено для удаления", accommodation_id
                )
            else:
