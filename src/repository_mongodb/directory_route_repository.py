...
class DirectoryRouteRepository(IDirectoryRouteRepository):
    ...
    async def add(self, directory_route: DirectoryRoute) -> DirectoryRoute:
        try:
            if (
                not directory_route.departure_city
                or not directory_route.destination_city
            ):
    ...
    async def update(self, update_directory_route: DirectoryRoute) -> None:
        try:
            ...
            if (
                not update_directory_route.departure_city
                or not update_directory_route.destination_city
            ):
                logger.error("Отсутствуют данные о городах")
                raise ValueError("Both cities are required")
            ...
            logger.error(
                "Ошибка при обновлении справочника маршрутов с ID %s: %s",
                update_directory_route.d_route_id,
                str(e),
                exc_info=True,
            )
            raise
    ...
    async def delete(self, directory_route_id: int) -> None:
        try:
            ...
            if result.deleted_count == 0:
                logger.warning(
                    "Справочник с ID %d не найден для удаления", directory_route_id
                )
            else:
    ...
    async def get_by_cities(
        ...
        try:
            ...
            logger.error(
                "Ошибка при получении справочника маршрутов по городам: %s",
                str(e),
                exc_info=True,
            )
            return None
