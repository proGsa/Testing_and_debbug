...
class RouteRepository(IRouteRepository):
    ...
    async def update(self, update_route: Route) -> None:
        try:
            ...
            if not update_route.d_route:
                logger.error(
                    "Отсутствуют данные о справочнике путешествий для обновления"
                )
                return
            ...
            if result.modified_count == 0:
                logger.warning(
                    "Маршрут с ID %d не найден для обновления", update_route.route_id
                )
            else:
    ...
    async def get_routes_by_travel_id_ordered(self, travel_id: int) -> list[Route]:
        try:
            routes = []
            async for doc in self.routes.find({"travel._id": travel_id}).sort(
                "start_time"
            ):
                d_route_doc = await self.d_route_repo.get_by_id(doc["d_route"]["_id"])
                travel_doc = await self.travel_repo.get_by_id(doc["travel"]["_id"])
        ...
        try:
            ...
            for route in all_routes:
                if (
                    ...
                    and city_id
                    in {route.departure_city.city_id, route.destination_city.city_id}
                ):
    ...
    async def delete_city_from_route(self, travel_id: int, city_id: int) -> None:
        try:
            ...
            for i, route in enumerate(routes):
                ...
                if city_id in {
                    (
                        route.d_route.departure_city.city_id
                        if route.d_route.departure_city
                        else None
                    ),
                    (
                        route.d_route.destination_city.city_id
                        if route.d_route.destination_city
                        else None
                    ),
                }:
            ...
            logger.error(
                "Ошибка при удалении города из маршрута: %s", str(e), exc_info=True
            )
            raise
    ...
    async def change_transport(
        ...
        try:
            ...
            if (
                not old_d_route
                or not old_d_route.departure_city
                or not old_d_route.destination_city
            ):
                logger.error("Справочник маршрута с ID %d не найден", d_route_id)
                return None

            new_d_route = await self.d_route_repo.get_by_cities(
                old_d_route.departure_city.city_id,
                old_d_route.destination_city.city_id,
                new_transport,
            )
            if (
                not new_d_route
                or not new_d_route.departure_city
                or not new_d_route.destination_city
            ):
                logger.error(
                    "Справочник маршрута с транспортом %s не найден", new_transport
                )
                return None
    ...
    async def _get_d_route_between(
        ...
        d_route_doc = await self.d_route_repo.get_by_cities(
        ...
        try:
            ...
            for route in routes:
                ...
                if (
                    ...
                    and route.d_route.destination_city.city_id == after_city_id
                ):
                    target_route = route
                    insert_after = True
                    break
                if (
                    ...
                    and route.d_route.departure_city.city_id == after_city_id
                ):
                    target_route = route
                    break
            ...
            if insert_after:
                ...
                if not d_route_new.departure_city or not d_route_new.destination_city:
                    raise ValueError(
                        "Город d_route_new.departure_city/d_route_new.destination_city не найден"
                    )
            ...
            logger.error(
                "Ошибка при вставке города в маршрут: %s", str(e), exc_info=True
            )
            raise
    ...
    async def get_routes_by_user_and_status_and_type(
        ...
        try:
            travel_ids = [
                t.travel_id
                for t in await self.travel_repo.get_travels_for_user(user_id, status)
            ]

            routes = []
            async for doc in self.routes.find(
                {"travel._id": {"$in": travel_ids}, "type": type_route}
            ):
                d_route_doc = await self.d_route_repo.get_by_id(doc["d_route"]["_id"])
                travel_doc = await self.travel_repo.get_by_id(doc["travel"]["_id"])
    ...
    async def get_route_parts(self, route_id: int) -> list[dict[str, Any]]:
        try:
            route_parts = []
            async for route_doc in self.routes.find({"_id": route_id}).sort(
                "start_time"
            ):
                logger.debug("route_doc %s/n", route_doc)
                d_route_doc = await self.d_route_repo.get_by_id(
                    route_doc["d_route"]["_id"]
                )
                if not d_route_doc:
                    continue
            ...
            logger.debug(
                "Получено %d частей маршрута для route_id=%d",
                len(route_parts),
                route_id,
            )
            return route_parts
            ...
            logger.error(
                "Ошибка при получении частей маршрута для route_id=%d: %s",
                route_id,
                str(e),
                exc_info=True,
            )
