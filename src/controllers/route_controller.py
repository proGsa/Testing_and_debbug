...
class RouteController:
    def __init__(
        ...
        ent_service: EntertainmentService,
        acc_service: AccommodationService,
    ) -> None:
    ...
    async def create_new_route_user(self, request: Request) -> dict[str, Any]:
        try:
            ...
            travel = Travel(
                ...
                entertainments=[
                    e
                    for eid in data.get("entertainments[]")
                    if (e := await self.ent_service.get_by_id(int(eid))) is not None
                ],
                accommodations=[
                    a
                    for aid in data.get("accommodations[]")
            ...
            raise HTTPException(
                status_code=500, detail=f"Ошибка при создании маршрута: {e!s}"
            )
    ...
    async def add_new_city(self, request: Request) -> dict[str, Any]:
        try:
            ...
            await self.route_service.insert_city_after(
                travel_id, new_city_id, after_city_id, transport
            )
            logger.info(
                "Город ID %d успешно добавлен в маршрут после города  %d",
                new_city_id,
                after_city_id,
            )
            ...
            logger.error(
                "Ошибка при добавлении города в маршрут: %s", str(e), exc_info=True
            )
    ...
    async def delete_city_from_route(self, request: Request) -> dict[str, Any]:
        try:
            ...
            if not city_id or not travel_id:
                logger.warning(
                    "Недостаточно данных в запросе: city_id=%s, travel_id=%s",
                    city_id,
                    travel_id,
                )
                return {"success": False, "message": "Требуются city_id и travel_id"}
            ...
            logger.info(
                "Город ID %d успешно удален из маршрута travel_id=%d",
                city_id,
                travel_id,
            )
            return {"success": True, "message": f"Город {city_id} удален из маршрута"}
    ...
    async def change_transport(self, route_id: int, request: Request) -> dict[str, Any]:
        try:
            ...
            if not route_id or not new_transport:
                logger.warning("Отсутствуют обязательные поля в запросе")
                return {"message": "Missing required fields in request"}
            new_route = await self.route_service.change_transport(
                d_route_id, route_id, new_transport
            )
            logger.info(
                "Транспорт в маршруте ID %d успешно изменен на %s",
                route_id,
                new_transport,
            )
            ...
            return {
                ...
                "d_route_id": new_route.d_route.d_route_id,
            }
        except Exception as e:
            logger.error(
                "Ошибка при изменении транспорта в маршруте ID %d: %s",
                route_id,
                str(e),
                exc_info=True,
            )
    ...
    async def get_route_details(self, request: Request) -> dict[str, Any]:
        try:
            ...
            logger.error(
                "Ошибка при получении информации о маршруте ID: %s",
                str(e),
                exc_info=True,
            )
        ...
        try:
            ...
            for r in route_list:
                if r and r.d_route and r.travels:
                    travel_users = await self.travel_service.get_users_by_travel(
                        r.travels.travel_id
                    )
                    routes.append(
                        {
                            ...
                            "d_route": (
                                {
                                    ...
                                    "departure_city": (
                                        r.d_route.departure_city.name
                                        if r.d_route.departure_city
                                        else None
                                    ),
                                    "destination_city": (
                                        r.d_route.destination_city.name
                                        if r.d_route.destination_city
                                        else None
                                    ),
                                }
                                if r.d_route
                            ...
                            "travel": (
                                {
                                    ...
                                    "entertainments": [
                                        ...
                                        for e in (
                                            await self.travel_service.get_entertainments_by_travel(
                                                r.travels.travel_id
                                            )
                                        )
                                    ],
                                    "accommodations": [
                                        ...
                                        for a in (
                                            await self.travel_service.get_accommodations_by_travel(
                                                r.travels.travel_id
                                            )
                                        )
                                    ],
    ...
    async def change_travel_duration(
        self, route_id: int, request: Request
    ) -> dict[str, Any]:
        try:
            data = await request.json()
            ...
            logger.error(
                "Ошибка при продлении маршрута ID %d: %s",
                route_id,
                str(e),
                exc_info=True,
            )
    ...
    async def join_to_trip(self, route_id: int, request: Request) -> dict[str, Any]:
        ...
        route.type = "Свои"
        if route.travels:
            current_users = await self.travel_service.get_users_by_travel(
        ...
        if route.travels and user:
            await self.travel_service.link_users(
                route.travels.travel_id,
