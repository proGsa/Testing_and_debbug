...
class TravelController:
    ...
    async def create_new_travel(self, request: Request) -> dict[str, Any]:
        try:
            ...
            entertainments = [
                e
                for eid in data["entertainment_ids"]
                if (e := await self.ent_service.get_by_id(eid)) is not None
            ]

            accommodations = [
                a
                for aid in data["accommodation_ids"]
    ...
    async def get_travel_details(self, travel_id: int) -> dict[str, Any]:
        try:
            ...
            entertainment_list = await self.travel_service.get_entertainments_by_travel(
                travel_id
            )
            accommodation_list = await self.travel_service.get_accommodations_by_travel(
                travel_id
            )
            ...
            logger.error(
                "Ошибка при получении информации о путешествии ID: %s",
                str(e),
                exc_info=True,
            )
    ...
    async def complete_travel(self, travel_id: int) -> dict[str, Any]:
        try:
            ...
            logger.error(
                "Ошибка при завершении путешествия ID %d: %s",
                travel_id,
                str(e),
                exc_info=True,
            )
    ...
    async def get_all_travels(self) -> dict[str, Any]:
        try:
            ...
            for t in travel_list:
                entertainment_list = (
                    await self.travel_service.get_entertainments_by_travel(t.travel_id)
                )
                accommodation_list = (
                    await self.travel_service.get_accommodations_by_travel(t.travel_id)
                )
