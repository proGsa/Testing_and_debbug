...
class UserController:
    ...
    async def get_user_profile(self, user_id: int) -> dict[str, Any]:
        try:
            ...
            logger.error("Ошибка при получении профиля пользователя ID: %s", str(e), exc_info=True)
    ...
    async def create_admin(self, request: Request) -> dict[str, Any]:
        try:
            ...
            return {"message": "Admin registered successfully", "user_id": registered_admin.user_id}
        except Exception as e:
            logger.error("Ошибка при регистрации администратора: %s", str(e), exc_info=True)
    ...
    async def delete_user(self, user_id: int) -> dict[str, Any]:
        try:
            ...
            logger.error("Ошибка при удалении пользователя ID %d: %s", user_id, str(e), exc_info=True)
    ...
    async def update_admin(self, user_id: int, request: Request) -> dict[str, Any]:
        try:
            ...
            if admin_old is None:
                return {"message": "User not found", "error": f"User with id {user_id} not found"}
