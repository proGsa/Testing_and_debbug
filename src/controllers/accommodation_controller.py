...
class AccommodationController:
    def __init__(
        self, accommodation_service: AccommodationService, city_service: CityService
    ) -> None:
    ...
    async def update_accommodation(
        self, accommodation_id: int, request: Request
    ) -> dict[str, Any]:
        try:
            data = await request.json()
            ...
            logger.error(
                "Ошибка при обновлении проживания ID %d: %s",
                accommodation_id,
                str(e),
                exc_info=True,
            )
    ...
    async def get_accommodation_details(
        ...
        try:
            ...
            if accommodation:
                logger.info(
                    "Проживание ID %d найдено: %s", accommodation_id, accommodation
                )
                return {
                    "accommodation": {
            ...
            logger.error(
                "Ошибка при получении информации о проживании: %s",
                str(e),
                exc_info=True,
            )
    ...
    async def get_all_accommodation(self) -> dict[str, Any]:
        try:
            ...
            logger.error(
                "Ошибка при получении списка проживаний: %s", str(e), exc_info=True
            )
    ...
    async def delete_accommodation(self, accommodation_id: int) -> dict[str, Any]:
        try:
            ...
            logger.error(
                "Ошибка при удалении проживания ID %d: %s",
                accommodation_id,
                str(e),
                exc_info=True,
            )
    ...
    async def update_accommodation_dates(
        ...
        try:
            ...
            new_price = int(
                self.calculate_price(accommodation.price / old_nights, nights)
            )
            ...
            logger.info(
                "Даты и цена проживания ID %d успешно обновлены", accommodation_id
            )
            return {
