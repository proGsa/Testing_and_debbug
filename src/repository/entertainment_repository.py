...
class EntertainmentRepository(IEntertainmentRepository):
    ...
    async def get_by_id(self, entertainment_id: int) -> Entertainment | None:
        ...
        try:
            ...
            if row:
                logger.debug("Найдено развлечение ID %d: %s", entertainment_id, row["event_name"])
    ...
    async def add(self, entertainment: Entertainment) -> Entertainment:
        ...
        try:
            if entertainment.city is None:
                ...
                raise ValueError("Невозможно добавить развлечение: отсутствуют данные о городе")
            ...
            logger.error("Ошибка при добавлении развлечения '%s': %s", entertainment.event_name, str(e), exc_info=True)
            await self.session.rollback()
        return entertainment
    ...
    async def update(self, update_entertainment: Entertainment) -> None:
        ...
        try:
            ...
            await self.session.commit()
            logger.debug("Развлечение ID %d успешно обновлено", update_entertainment.event_name)
        except SQLAlchemyError as e:
