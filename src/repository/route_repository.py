...
class RouteRepository(IRouteRepository):
    def __init__(self, session: AsyncSession, d_route_repo: IDirectoryRouteRepository, travel_repo: ITravelRepository):
    ...
    async def update(self, update_route: Route) -> None:
        ...
        try:
            ...
            await self.session.rollback()
            logger.error("Ошибка при обновлении маршрута ID %d: %s", update_route.route_id, str(e), exc_info=True)
    ...
    async def delete_city_from_route(self, travel_id: int, city_id: int) -> None:
        ...
        for i, route in enumerate(routes):
            ...
            if ((route.d_route.departure_city and route.d_route.departure_city.city_id == city_id) or
                (route.d_route.destination_city and route.d_route.destination_city.city_id == city_id)):
        ...
        if first_segment_idx > 0:
            ...
            if prev_route.d_route is not None and prev_route.d_route.departure_city is not None:
        ...
        if last_segment_idx < len(routes) - 1:
            ...
            if next_route.d_route is not None and next_route.d_route.destination_city is not None:
    ...
    async def change_transport(self, d_route_id: int, route_id: int, new_transport: str) -> Route | None:
        try:
            ...
            new_d_route = await self.d_route_repo.change_transport(d_route_id, new_transport)
            if not new_d_route:
                logger.error("Справочник маршрута с ID %d и транспортом %s не найден", d_route_id, new_transport)
                return None
            ...
            result = await self.session.execute(update_query, {
                "new_d_route_id": new_d_route.d_route_id,
                "route_id": route_id
            })
            await self.session.commit()
    ...
    async def _get_d_route_between(self, from_city_id: int, to_city_id: int, transport: str) -> DirectoryRoute:
        d_route = await self.d_route_repo.get_by_cities(from_city_id, to_city_id, transport)
        if not d_route:
    ...
    async def insert_city_after(self, travel_id: int, new_city_id: int, after_city_id: int, transport: str) -> None:
        ...
        for route in routes:
            ...
            dest_city = getattr(route.d_route, 'destination_city', None) or getattr(route.d_route, 'destination_city', None)
    ...
    async def get_routes_by_user_and_status_and_type(self, user_id: int, status: str, type_route: str) -> list[Route]:
        try:
            ...
            """)
            result = await self.session.execute(sql, {"user_id": user_id, "status": status, "type": type_route})
            rows = result.mappings().all()
    ...
    async def get_route_parts(self, travel_id: int) -> list[dict[str, Any]]:
        ...
        try:
            ...
            logger.debug("Получено %d частей маршрута для travel_id=%d", len(route_parts), travel_id)
            return route_parts
            
        except SQLAlchemyError as e:
            logger.error("Ошибка при получении частей маршрута для travel_id=%d: %s", 
                    travel_id, str(e), exc_info=True)
            raise